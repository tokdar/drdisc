...
bdregjump_adapt_tempered <- function(y, x=1, b=NULL, nsamp=100, thin=1, burn=100, 
                                     ...
                                     print.process = FALSE, print.temp = FALSE, print.i = FALSE){
...
bdregjump_adapt_tempered_fix <- function(y, x=1, b=NULL, nsamp=100, thin=1, burn=100, 
                                     shapes=c(1,1), prec=1, order = 15, 
                                     jump=list(a=NULL, persistence=0.5, positive=TRUE,
                                               prec=1, ncand=10, update.jbw=TRUE),
                                     adap=list(mu_adap=NULL, Sigma_adap=NULL,
                                               ...
                                               chol.pivot=FALSE, 
                                               parallel = 5),
                                     print.process = FALSE, print.swap = FALSE, print.i = FALSE){
  ...
  temp_PT <- c(1, cumprod(rep(temp_end^(1/(L-1)), L-1)))
  ...
  for(iter in -(burn-1):(nsamp*thin)){
    ...
    if(print.swap) print(swap_accept_prob)
  ...
  return(list(b=b.store, a_PT=a_PT.store, jbw=jbw.store, Ntot=Ntot.store,
              ...
              shapes=shapes.store, p_accept = p_accept.store,
              temp = temp_PT.store, swap_info = swap_info.store,
              log.likelihood_PT = log.likelihood_PT.store,
              swap_accept_prob = swap_accept_prob.store,
              runtime=run.time, 
              acpt=c(a=nacpt, store=nacpt.bp, swap=nswap)/(burn+nsamp*thin))
  )
}
...
bdregjump_adapt_onetemp <- function(y, x=1, b=NULL, nsamp=100, thin=1, burn=100, 
                            shapes=c(1,1), prec=1, order = 5, temp = 0.5,
                            jump=list(a=NULL, persistence=0.5, positive=TRUE, 
                                      prec=1, ncand=10, update.jbw=TRUE),
                            adap=list(mu_adap=NULL, Sigma_adap=NULL,
                                      alpha_star = 0.234, r_adap = 2/3,
  ...
  get.miss.reject.x <- function(b, n, x, a, shapes=c(1,1), jbw=0.16, temp, positive=FALSE){
    n.hits <- 0
    n.miss <- 0
    Poly.miss <- NULL
    Half.miss <- NULL
  ...
  for(iter in -(burn-1):(nsamp*thin)){
    ...
    missing.stuff <- get.miss.reject.x(b, n, x, a, shapes, jbw, temp, positive)
    n.miss <- missing.stuff$n.miss
    Poly.miss <- missing.stuff$Poly.miss
    Half.miss <- missing.stuff$Half.miss
    w.miss <- missing.stuff$w.miss
...
### No conditonal on polya-gamma

bdregjump_adapt_noPG <- function(y, x=1, b=NULL, nsamp=100, thin=1, burn=100, 
                            shapes=c(1,1), prec=1, order = 5,
                            jump=list(a=NULL, persistence=0.5, positive=TRUE, 
                                      prec=1, ncand=10, update.jbw=TRUE),
                            adap=list(mu_adap=NULL, Sigma_adap=NULL,
  ...
  for(iter in -(burn-1):(nsamp*thin)){
    ...
    loglike.new <- get.log.likelihood(a.new)
    loglike <- get.log.likelihood(a)
...
bdregjump_noa_trimr <- function(y, x=1, b=NULL, nsamp=100, thin=1, burn=100, 
                                shapes=c(1,1), prec=1, order = 5, right_end = 0.5,
                                print.process = FALSE, print.i = FALSE){
  ...
  get.miss.reject.x <- function(b, n, x, shapes=c(1,1), length_trim, print.n.remain = FALSE){
    n.hits <- 0
    n.miss <- 0
    Poly.miss <- NULL
    w.miss <- NULL
    ...
    return(list(Poly.miss=as.matrix(Poly.miss),
                w.miss=w.miss, n.miss=n.miss, x.miss=x.miss, y.miss=y.miss))
  }
  ...
  for(iter in -(burn-1):(nsamp*thin)){
    ...
    missing.stuff <- get.miss.reject.x(b, n, x, shapes, length_trim)
    n.miss <- missing.stuff$n.miss
    Poly.miss <- missing.stuff$Poly.miss
    w.miss <- missing.stuff$w.miss
    x.miss <- missing.stuff$x.miss
  ...
  run.time <- proc.time() - time.stamp.0
  return(list(b=b.store, Ntot=Ntot.store, shapes=shapes.store, runtime=run.time, 
              acpt=c(a=nacpt, store=nacpt.bp)/(burn+nsamp*thin))
  )
}


bdregjump_adapt_trimr <- function(y, x=1, b=NULL, nsamp=100, thin=1, burn=100, 
                            shapes=c(1,1), prec=1, order = 5, right_end = 0.25,
                            jump=list(a=NULL, persistence=0.5, positive=TRUE, 
                                      prec=1, ncand=10, update.jbw=TRUE),
                            adap=list(mu_adap=NULL, Sigma_adap=NULL,
                                      alpha_star = 0.234, r_adap = 2/3,
...
########### new basis functions (polynomial) & symmetric trimming ##########


bdregjump_noa_poly_trim <- function(y, x=1, b=NULL, nsamp=100, thin=1, burn=100, 
                                shapes=c(1,1), prec=1, trim = 0.5, order = 3,
                                print.process = FALSE, print.i = FALSE){
  lpoly <- list(
    #  Vectorize(function(x) return(1/sqrt(2)), "x"), ## k = 0
    function(x) return(sqrt(3/2)*x), ## k = 1
  ...
  get.miss.reject.x <- function(b, n, x, shapes=c(1,1), trim, print.n.remain = FALSE){
    n.hits <- 0
    n.miss <- 0
    Poly.miss <- NULL
    w.miss <- NULL
  ...
  for(iter in -(burn-1):(nsamp*thin)){
    ...
    missing.stuff <- get.miss.reject.x(b, n, x, shapes, trim)
    n.miss <- missing.stuff$n.miss
    Poly.miss <- missing.stuff$Poly.miss
    w.miss <- missing.stuff$w.miss
    x.miss <- missing.stuff$x.miss
...
bdregjump_adapt_poly_trim <- function(y, x=1, b=NULL, nsamp=100, thin=1, burn=100, 
...
#### no Polya-Gamma data augmentation ####

bdregjump_noa_poly_trim_noPG <- function(y, x=1, b=NULL, nsamp=100, thin=1, burn=100, 
                                    shapes=c(1,1), prec=1, trim = 0.5, order = 3,
                                    print.process = FALSE, print.i = FALSE){
  lpoly <- list(
    #  Vectorize(function(x) return(1/sqrt(2)), "x"), ## k = 0
  ...
  get.log.likelihood <- function(y, x, Poly.obs, Poly.miss,
    ...
    w.obs <- rowSums(x * (Poly.obs %*% b))
    w.miss <- rowSums(x.miss * (Poly.miss %*% b))
...
bdregjump_adapt_poly_trim_noPG <- function(y, x=1, b=NULL, nsamp=100, thin=1, burn=100, 
                                      shapes=c(1,1), prec=1, order = 3, trim = 0.5,
                                      jump=list(a=NULL, persistence=0.5, positive=TRUE, 
                                                prec=1, ncand=10, update.jbw=TRUE),
                                      adap=list(mu_adap=NULL, Sigma_adap=NULL,
  ...
  mu_adap <- adap$mu_adap
  if(is.null(mu_adap)) mu_adap <- rep(0,p)
  Sigma_adap <- adap$Sigma_adap
  if(is.null(Sigma_adap)) Sigma_adap <- diag(p)
  alpha_star <- adap$alpha_star
  if(is.null(alpha_star)) alpha_star <- 0.234
  ...
  loglambda_adap_b <- rep(loglambda_adap, order)
  mu_adap_b <- replicate(order, mu_adap)
  Sigma_adap_b <- replicate(order, Sigma_adap)
  ...
  random_walk_step <- function(y, x, prec, Poly.obs, b_order,
                               positive = TRUE, alpha_star, r_adap, chol.pivot, 
                               b, a, shapes, jbw,
                               gamma_adap, loglambda_adap, mu_adap, Sigma_adap){
    nacpt <- 0
    ...
    if(chol.pivot == TRUE){
      var_order <- attr(Sigma_chol, "pivot")
      bz.new[var_order] <- crossprod(Sigma_chol, rnorm(p))
    } else{
      bz.new <- crossprod(Sigma_chol, rnorm(p))
    }
    
    b.new <- b
    b.new[b_order,] <- b.new[b_order,] + bz.new
    ...
    log.hastings.ratio <- ( (loglike.new - loglike) +
                              sum(dnorm(b.new,0,sqrt(1/prec),log=TRUE)
                                  - dnorm(b,0,sqrt(1/prec),log=TRUE)) )
    ...
    return(list(b = b, loglambda_adap = loglambda_adap, mu_adap = mu_adap,
                Sigma_adap = Sigma_adap, nacpt = nacpt))
  }
  ...
  for(iter in -(burn-1):(nsamp*thin)){
    ...
    for(b_order in 1:order){
      
      random_walk_result <- random_walk_step(y=y, x=x, prec=prec, Poly.obs = Poly.obs,
                                             b_order = b_order,
                                             positive=positive, alpha_star=alpha_star,
                                             r_adap=r_adap, chol.pivot=chol.pivot, 
                                             b=b, a=a,
                                             shapes=shapes, jbw=jbw,
                                             gamma_adap=gamma_adap,
                                             loglambda_adap=loglambda_adap_b[b_order],
                                             mu_adap=mu_adap_b[,b_order], 
                                             Sigma_adap=Sigma_adap_b[,,b_order])
      b[b_order,] <- random_walk_result$b[b_order,]
      loglambda_adap_b[b_order] <- random_walk_result$loglambda_adap
      mu_adap_b[,b_order] <- random_walk_result$mu_adap
      Sigma_adap_b[,,b_order] <- random_walk_result$Sigma_adap
      nacpt_b[b_order] <- nacpt_b[b_order] + random_walk_result$nacpt
    }
